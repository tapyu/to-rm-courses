...
We have found a restriction here: we have not defined a matrix-vector differentiation for the numerator layout. Since \(\frac{\partial f(\mathbf{x})}{\partial \mathbf{x}}\) is a \(1\times n\) matrix, we cannot continue. Indeed, as we will see after, the notation of the Hessian matrix in Eq.\eqref{eq:hessian-eq} follows the denominator layout, not the numerator.
