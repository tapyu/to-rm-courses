...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(T_{s}\) & Sampling time interval\\
    \(R\) & Bit rate\\
    \(T\) & Bit duration\\
    \(T_c\) & Chip duration\\
    \(T_{sy}, T_{sym}\) & Symbol duration\\
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(\because\) & Because\\
    \(\mid, :\) & Such that, sometimes that parantheses is used\\
    \(, , \left( \cdot \right)\) & Used to separate the quantifier with restricted domain from the propositional function, e.g., \(\forall \; x < 0 \left( x^{2} > 0 \right)\) or \(\forall \; x < 0, x^{2} > 0\)\\
    \(\therefore\) & Therefore\\
\end{xltabular}
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(a \wedge b\) & Conjunction (logical AND) operator between \(a\) and \(b\)\\
    \(a \vee b\) & Disjunction (logical OR) operator between \(a\) and \(b\)\\
    \(a \oplus b\) & Exclusive OR (logical XOR) operator between \(a\) and \(b\)\\
    \(a \rightarrow b\) & Implication (or conditional) statement\\
    \(a \leftrightarrow b\) & Bi-implication (or biconditional) statement, i.e., \(\left( a \rightarrow b \right) \wedge (b \rightarrow a )\) \\
    \(a \equiv b, a \iff b, a \Leftrightarrow b \) & Logical equivalence, i.e., \(a \leftrightarrow b\) is a tautology
\end{xltabular}
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(a \backslash b\) & \(b\) is a positive integer multiple of \(a\), i.e., \( \exists\; n \in \mathbb{Z}_{++} \mid b = n a \) \\
    \(a \centernot\backslash b\) & \(b\) is not a positive integer multiple of \(a\), i.e., \( \nexists\; n \in \mathbb{Z}_{++} \mid b = n a \) \\
