...
\section{Signals and functions operations}
\begin{xltabular}{\textwidth}{XX}
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(\mathbf{Q}\) & Eigenvectors matrix; Orthogonal matrix of the QR\\
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(\sigma_x\) & Variance of the random variable \(x\)\\
...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(p(\cdot), f(\cdot)\) & Probability density function (PDF)\\
    ...
    \(M_x(t), \Phi_x(-j t), E\left[ e^{t x} \right]\) & Moment-generating function (MGF) of \(x\) (\(M_x(t) = \Phi_x(-j t)\) and \(\Phi(\omega) = M_x(j\omega)\))\\
    \(\Psi_x(\omega), \ln \Phi_x(\omega), \ln E\left[ e^{j \omega x}\right]\) & Second characteristic function, i.e., \(\ln E\left[ e^{j \omega x} \right]\)\\
