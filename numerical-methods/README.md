...
[**Disambiguation**][43]: In the domain of digital signal processing (DSP), the term interpolation refers to the process of converting a discrete-time signal to a higher sampling rate (Upsampling) using various digital filtering techniques (for example, convolution with a frequency-limited impulse signal). Therefore, in the context of DSP, "interpolating" can mean obtaining an upsampled discrete-time signal, while in numerical methods fields, it means obtaining a continuous function that passes through a set of samples.
