...
The fuzzy system consists of a continuum-valued logic in which the truth value may be in the interval between 0 and 1. This contrasts with the so-called ordinary sets, that has only two characteristic functions (0 or 1). This approach enables to model the vagueness or uncertainty encountered in many real problems.

The fuzzy model maps the input variable domain to the \(\left[ 0,1 \right]\) counter-domain through a membership function, This process is commonly referred as fuzzyfication. Based on a set of rules, the fuzzy system uses the truth values of the fuzzyfied input to infer how much the antecedents impact the consequent.

There are many different possibilities to define a fuzzy rule. A well-established fuzzy system model the is Mamdani type, which can be generically written as
\begin{align}
    &\text{IF } x_n \text{ IS } A_1 \text{ AND } x_n \text{ IS } A_2 ... \text{ AND } x_n \text{ IS } A_I \nonumber \\
    &\text{ THEN } y_n \text{ IS } B_1, \nonumber
\end{align}
where \(B_1\) and \(\left\{ A_1, A_2, \cdots, A_I \right\}\) are the output and input fuzzy sets, respectively, and ``AND'' is a fuzzy operator. Among the different infer methods, the Modus Ponens, defined by,

is widely used.

The results are then ``defuzzyfied'' to a crisp value by one of several methods. A very popular method is the ``centroid'', which decides by the center of mass of the output fuzzy set.
