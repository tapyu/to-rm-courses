...
In these equations, \(v_{j}^{(l)}(n)\) and \(y_{j}^{(l)} (n)\) are the induced local field and the output signal of \(j\)th neuron on the \(l\)th layer, being \(y_{0}^{(l)} (n) \triangleq -1\) the input bias for the neurons on the layer \(l+1\).
        ...
        \ForAll{\(\left\{ 1, 2, \cdots, N_e \right\}\)}{
            \ForAll{Instancies in the training dataset}{
                ...
                \For{\(l \in \left\{ L-1, L-2, ..., 1 \right\}\)}{
                    ...
                    \(\mathbf{W}^{(l)}(n+1) = \mathbf{W}^{(l)}(n) + \eta\boldsymbol{\deltaup}^{(l)}(n) \mathbf{y}^{(l-1)}(n)^\trans.\)
                }
            }
            \(\mathbf{X}_{trn}, \mathbf{D}_{trn} \leftarrow \text{shuffle}\)
        }
...
\begin{algorithm}[H]
    \DontPrintSemicolon
      ...
      \KwInput{\(\mathbf{X}, K\) \tcc*{\(\mathbf{X}\) does not have the test instances}}
    ...
    \(\mathbf{X} \leftarrow\) shuffle
      ...
      \ForAll{Set of hyperparameters}{
        ...
        \For{\(k \in \left\{ 1, 2, ..., K \right\}\)}{
            ...
            \(\mathbf{X}_{trn}, \mathbf{X}_{val} \leftarrow\) select the \(k\)th division of training and validation dataset.
            

            \ForAll(\tcc*[h]{for each epoch}){\(\left\{ 1, 2, ..., N_e \right\}\)}{
                \(\mathbf{X}_{trn} \leftarrow\) shuffle

                \(\mathbf{W}(n) \leftarrow \) train with a given set of hyperparameters
            }

            Save the accuracy for the trained \(\mathbf{W}(n)\) for the dataset \(\mathbf{X}_{val}\)
