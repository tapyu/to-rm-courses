...
\end{itemize} & \vspace{-3.5ex} \begin{itemize}[leftmargin=*]
    ...
    \item \(\mathbf{x} \in S\) is the \emph{minimal} element of \(S\) with respect to the proper cone \(K\) if \(\mathbf{y} \preceq_K \mathbf{x}\) only when \(\mathbf{y} = \mathbf{x}\) (for the \emph{maximal}, \(\mathbf{y} \succeq_K \mathbf{x}\) only when \(\mathbf{y} = \mathbf{x}\)). It means that \((\mathbf{x} - K) \cap S = \left\{ \mathbf{x} \right\}\) for minimal (for the maximal \((\mathbf{x} + K) \cap S = \left\{ \mathbf{x} \right\}\)), where \(\mathbf{x} - K\) denotes the reflected set \(K\) shift by \(\mathbf{x}\). Note that any point in \(\mathbf{x} - K\) is comparable with \(\mathbf{x}\) and is lower or equal to \(\mathbf{x}\) in the generalized inequality mean. The set \(S\) can have many different minimal (maximal) elements.
