...
\begin{xltabular}[l]{0.5\linewidth}{|X|X|}
    ...
    \end{itemize} & It is neither convex nor concave in most of the cases\\
    \hline
...
\section{Fuctions}
\subsection{Categories of functions regarding its curvatuve}
\begin{itemize}
    \item In CVX, for functions with multiple arguments (a vector as input), the curvature categories are always considered jointly \autocite{DCPRulesetCVX}.
...
\end{itemize} & \vspace{-3.5ex} \begin{itemize}[leftmargin=*]
    ...
    \end{itemize} & \vspace{-3.5ex}
    \begin{itemize}[leftmargin=*]
        \item Note that it is guaranteed to be convex iff the base power is solely \(x\). For instance, \((x+1)^2\) is convex, but \((x-1)^2\) is not.
    \end{itemize}\\
    \hline
...
\begin{itemize}
    ...
    \item It will \emph{not} check, for example, sums of products of affine expressions. For example, \(\mathtt{x \textsuperscript{$\wedge$} 2 + 2 * x * y + y \textsuperscript{$\wedge$}2}\), where \(\mathtt{x,y} \in \mathbb{R}\), will cause an error in CVX, because the second term is neither convex nor concave. However, the alternative expressions \(( x + y ) \textsuperscript{$\wedge$} 2\) and \(( x + y ) * ( x + y )\) are compatible to CVX.
...
\subsection{Construction examples of CVX-compatible expressions}
The composition of convex functions is the base rule for the construction of expressions on CVX \autocite{DCPRulesetCVXa}. One shall use the atoms functions in order to build expressions on CVX \autocite{DCPRulesetCVX}. Consider the following examples:
    \begin{itemize}
        \item \(f = \mathtt{max}(\mathtt{abs}(\mathbf{x}))\)
        \begin{itemize}
        ...
        \end{itemize}
        \item \(f = \mathtt{sqrt}(\inner{\mathbf{k},\mathbf{x}}) + \mathtt{min}(4, 1.3-\mathtt{norm}(\mathbf{A}*\mathbf{x}-\mathbf{b}))\), where \(\mathbf{k}, \mathbf{A}, \mathbf{b}\) are constants.
        \begin{itemize}
        ...
        \end{itemize}
        \item \(f = (x\textsuperscript{$\wedge$}2 + 1)\textsuperscript{$\wedge$}2\)
        \begin{itemize}
            ...
            \item To circumvent it, one can rewrite as \(f\) as \(x\textsuperscript{$\wedge$}4 + 2*x\textsuperscript{$\wedge$}2 + 1\). Now, the power function guarantees that \(f\) is convex, thus this expression is CVX-compatible.
