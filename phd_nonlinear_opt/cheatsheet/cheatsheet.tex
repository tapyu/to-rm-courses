...
\section{Table of the known sets}
\begin{table}[H]
...
\begin{itemize}
    ...
    \item If \(\nabla f(\mathbf{x}) = \mathbf{0}\), then \(f(\mathbf{y})\geq f(\mathbf{x}),\forall\;\mathbf{y}\in\dom{f}\) and \(\mathbf{x}\) is the global minimum.
...
\begin{itemize}
    ...
    \item If \(\nabla f(\mathbf{x}) = \mathbf{0}\), then \(f(\mathbf{y})\geq f(\mathbf{x}),\forall\;\mathbf{y}\in\dom{f}\) and \(\mathbf{x}\) is the global maximum.
...
\begin{itemize}
    \item Nonconvex and nonconcave functions do not attend the convexity or concavity rule
\end{itemize}
