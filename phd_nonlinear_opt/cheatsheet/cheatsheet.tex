...
\usepackage[margin=2mm,textwidth=155mm,paperheight=150cm,paperwidth=30cm]{geometry}
...
\end{itemize} & \vspace{-3.5ex} \begin{itemize}[leftmargin=*]
    ...
    \item \(\mathbf{x} \in S\) is the \emph{minimum} element of \(S\) with respect to the convex cone \(K\) if \(\mathbf{x} \preceq_K \mathbf{y}, \;\forall\;\mathbf{y} \in S\) (for \emph{maximum}, \(\mathbf{x} \succeq_K \mathbf{y}, \;\forall\;\mathbf{y} \in S\)). It means that \(S \subseteq \mathbf{x} + K\) (for the maximum, \(S \subseteq \mathbf{x} - K\)), where \(\mathbf{x} + K\) denotes the set \(K\) shifted from the origin by \(\mathbf{x}\). Note that any point in \(K+\mathbf{x}\) is greater or equal to \(\mathbf{x}\) in the generalized inequality mean. The set \(S\) does not necessarily have a minimum (maximum), but the minimum (maximum) is unique if it does.
