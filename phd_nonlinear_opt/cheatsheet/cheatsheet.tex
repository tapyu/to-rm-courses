...
\usepackage[margin=2mm,textwidth=155mm,paperheight=130cm,paperwidth=30cm]{geometry}
...
\(\alpha\)-sublevel set:
\begin{itemize}[leftmargin=*]
    \item \(C_\alpha = \{\mathbf{x} \in \dom{f} \mid f(\mathbf{x}) \leq \alpha\}\) (regarding convexity)
    \item \(C_\alpha = \{\mathbf{x} \in \dom{f} \mid f(\mathbf{x}) \geq \alpha\}\) (regarding concavity)
\end{itemize} & \vspace{-3.5ex}
\begin{itemize}[leftmargin=*]
    \item If \(f\) is a convex function, then sublevel sets of \(f\) are convexes for any \(\alpha\in \mathbb{R}\).
...
\end{tabularx}
    \begin{tabularx}{\textwidth}{|>{\setlength\hsize{1\hsize}\setlength\linewidth{\hsize}}X|>{\setlength\hsize{.9\hsize}\setlength\linewidth{\hsize}}X|>{\setlength\hsize{1.1\hsize}\setlength\linewidth{\hsize}}X|}%{| >{\hsize=.5\hsize}X | >{\hsize=1.5\hsize}X |}
        ...
        \begin{itemize}[leftmargin=*]
            ...
            \begin{itemize}[label=\(\triangleright\)]
                \item \(f\) is convex if \(h\) is convex and nondecreasing in each argument of \(\mathbf{x}\), and \(\left\{ g_i \right\}_{i=1}^{k}\) is a set of convex functions.
                \item \(f\) is convex if \(h\) is convex and nonincreasing in each argument of \(\mathbf{x}\), and \(\left\{ g_i \right\}_{i=1}^{k}\) is a set of concave functions.
                \item \(f\) is concave if \(h\) is concave and nondecreasing in each argument of \(\mathbf{x}\), and \(\left\{ g_i \right\}_{i=1}^{k}\) is a set of concave functions.
            \end{itemize}
        \end{itemize} & \vspace{-3.5ex}
        \begin{itemize}[leftmargin=*]
        ...
        \item For scale composition, the remarkable ones are:
            \begin{itemize}[label=\(\triangleright\)]
                ...
                \item If \(g\) is convex then \(f(\mathbf{x}) = h(g(\mathbf{x})) = - \log{\left( -g(x) \right)}\) is convex on \(\left\{ x\mid g(x)<0 \right\}\).
            \end{itemize}
        \end{itemize}\\
        \hline
        ...
        \end{itemize} & \vspace{-3.5ex} \begin{itemize}[leftmargin=*]
            \item Special cases is when \(f = w f\) (a nonnegative scalar factor) and \(f = f_1 + f_2\) (sum).
        \end{itemize}\\
        \hline
