...
\begin{tikzpicture}[
    ...
    \node[main, align=center]      (deterministic)  [left of=opt, xshift=-2.5cm]  {Deterministic Methods\\ $\bullet$ Make use of derivatives\\ $\bullet$ Unique solution \\ $\bullet$ Less computational effort};
    \node[main, align=center]      (stochasticmet)  [right of=opt, xshift=2.5cm]  {Stochastic Methods\\ $\bullet$ Make no use of derivatives\\ $\bullet$ May have many solutions \\ $\bullet$ Higher computational effort};
    ...
    % underuncert branch
    \node[uncertainties, align=center]      (underuncert)   [below of=stochasticmet, right of=stochasticmet, xshift=8cm, yshift=-1cm] {Optimization\\Under\\Uncertainties};
    ...
    \node[metaheuristic, align=center]      (swarmintelligence)   [above of=populationsearch, left of=populationsearch, xshift=-2cm, yshift=2cm] {Swarm\\Intelligence};
    \node[metaheuristic, align=center]      (evolutionary)   [above of=populationsearch, right of=populationsearch, xshift=-2cm, yshift=2cm] {Evolutionary\\Algorithm};
    ...
    \node[adapfilt, align=center]      (wiener)   [below of=adapfilt] {Wiener Filter\\(Optimum solution)};
    \node[adapfilt, align=center]      (steepestdescent)   [below of=wiener, left of=wiener] {Steepest Descent Algorithm\\(nonpractical)};
    \node[adapfilt, align=center]      (newtown)   [below of=wiener, right of=wiener] {Newtown Algorithm\\(nonpractical)};
    \node[adapfilt, align=center]      (stochasticgradient)   [below of=steepestdescent, right of=steepestdescent] {Stochastic Gradient Descent\\(SGD) family};
    \node[adapfilt, align=center]      (lms)   [below of=stochasticgradient, right of=stochasticgradient] {Least-Mean Square\\(LMS) Algorithm};
    \node[adapfilt, align=center]      (lmf)   [below of=stochasticgradient, left of=stochasticgradient] {Least-Mean Fourth\\(LMF) Algorithm};
    ...
    \node[adapfilt, align=center]      (lms-newtown)   [right of=lms, below of=newtown] {LMS-Newtown Algorithm};
    \node[adapfilt, align=center]      (rls)   [below of=steepestdescent, left of=steepestdescent] {Recursive Least\\Square (RLS) Algorithm};
    ...
    (wiener) edge node [] {} (steepestdescent)
    (wiener) edge node [] {} (newtown)
    (steepestdescent) edge node [] {} (rls)
    (steepestdescent) edge node [] {} (stochasticgradient)
    (stochasticgradient) edge node [] {} (lms)
    (stochasticgradient) edge node [] {} (lmf)
    (lms) edge node [] {} (nlms)
    (nlms) edge node [] {} (apa)
    (lms) edge node [] {} (lms-newtown)
    (newtown) edge node [] {} (lms-newtown)
